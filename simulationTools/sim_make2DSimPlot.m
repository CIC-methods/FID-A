...
% cell array where each element of the cell array is a simulated spectrum
...
if X<2 && Y<2
    temp1=op_freqrange(in{1},ppmmin,ppmmax);
    temp2=temp1;
elseif X<2 && Y>=2
    temp1=op_freqrange(in{floor(X/2)}{ceil(Y/2)},ppmmin,ppmmax);
    temp2=op_freqrange(in{floor(X/2)}{ceil(Y/2)},ppmmin,ppmmax);
elseif X>=2 && Y<2
    temp1=op_freqrange(in{ceil(X/2)},ppmmin,ppmmax);
    temp2=op_freqrange(in{ceil(X/2)},ppmmin,ppmmax);
elseif X>=2 && Y>=2
...
for x=1:size(in,1)
    for y=1:size(in,2)
        ...
        if Y<2
            tempSpec=op_freqrange(in{x},ppmmin,ppmmax);
        else
