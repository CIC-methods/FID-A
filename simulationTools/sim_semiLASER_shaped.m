% sim_semiLASER_shaped.m
% Dana Goerzen (McGill University, 2019).
% 
% USAGE:
%  out = sim_semiLASER_shaped(n,sw,Bfield,linewidth,sys,te,RF,tp,dx,dy,Gx,Gy,flipAngle,centreFreq)
% 
% DESCRIPTION:
...
% Unwanted coherences are removed using coherence order filtering.
...
function out = sim_semiLASER_shaped(n,sw,Bfield,linewidth,sys,te,RF,tp,dx,dy,Gx,Gy,flipAngle,centreFreq)
...
d=sim_shapedRF(d,H,RFX,tp,flipAngle,0,dx,Gx);           %2nd shaped 180 degree adiabatic refocusing pulse along X gradient
d=sim_COF(H,d,-1);                                      %Select coherence order -1
d=sim_evolve(d,H,tau2/1000);                            %Evolve by tau2
d=sim_shapedRF(d,H,RFY,tp,flipAngle,0,dy,Gy);           %3rd shaped 180 degree adiabatic refocusing pulse along Y gradient
d=sim_COF(H,d,1);                                       %Select coherence order 1
d=sim_evolve(d,H,tau2/1000);                            %Evolve by tau2
d=sim_shapedRF(d,H,RFY,tp,flipAngle,0,dy,Gy);           %4th shaped 180 degree adiabatic refocusing pulse along Y gradient
d=sim_COF(H,d,-1);                                      %Select coherence order -1
d=sim_evolve(d,H,tau1/1000);                            %Evolve by tau1
