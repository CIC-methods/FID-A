...
wrt=input('write? ','s');
if wrt=='y' || wrt=='Y'
    RF=io_writelcm(out,[filestring '/' filestring '_DriftCorr'],8.5);
    RF=io_writelcm(out_noproc,[filestring '/' filestring '_noDriftCorr'],8.5);
    if exist(filename2)
        RF=io_writelcm(out_w,[filestring '_w/' filestring '_w_DriftCorr'],8.5);
        RF=io_writelcm(out_w_noproc,[filestring '_w/' filestring '_w_noDriftCorr'],8.5);
    end
end
