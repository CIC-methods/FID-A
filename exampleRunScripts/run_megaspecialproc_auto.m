% run_megaspecialproc_auto.m
% Jamie Near, Sunnybrook Research Institute 2022.
% 
% USAGE:
% [diffSpec,sumSpec,subSpec1,subSpec2,outw]=run_megaspecialproc_auto(filestring,coilcombos,avgAlignDomain,alignSS);
% 
% DESCRIPTION:
% Fully automated processing script for Siemens MEGA-SPECIAL MRS data in .dat 
...
%                     2 - Perform alignment of edit-on and edit-off subspectra.
%                     4 - Align all four sub-spectra (this option is not
%                     recommended for now).  
% 
% OUTPUTS:
% diffSpec           = Fully processed difference spectrum.
% sumSpec            = Fully processed sum spectrum.
% subSpec1           = Fully processed MEGA-SPECIAL subspectrum #1.
% subSpec2           = Fully processed MEGA-SPECIAL subspectrum #2.
% outw               = Fully processed water-unsuppressed spectrum.

function [diffSpec,sumSpec,subSpec1,subSpec2,outw]=run_megaspecialproc_auto(filestring,coilcombos,avgAlignDomain,alignSS)
...
if water
    ...
    if ~ccGiven
        coilcombos=op_getcoilcombos(op_averaging(op_combinesubspecs(op_fourStepCombine(raw),'summ')),1);
    end
...
out_noproc=op_combinesubspecs(op_fourStepCombine(op_averaging(out_cc)),'diff');
...
%Time to combine the MEGA-SPECIAL sub-spectra
if alignSS~=4
    out_ms=op_fourStepCombine(out_cc);
    if water
        outw_ms=op_fourStepCombine(outw_cc);
    end
else
    out_ms=out_cc;
    if water
...
if rmbadav=='n' || rmbadav=='N'
    ...
    while sat=='n' || sat=='N'
        ...
        nBadAvgTotal=0;
        out_ms2=out_ms;
        trackRemovedAvg = 1:out_ms.sz(out_ms.dims.averages);
        while nbadAverages>0
            [out_rm,metric{iter},badAverages]=op_rmbadaverages(out_ms2,nsd,'t');
        ...
        h=figure('visible','off');
        subplot(2,2,1);
        plot(out_ms.ppm,real(out_ms.specs(:,:,1)));xlim([1 5]);
        set(gca,'FontSize',8);
        set(gca,'XDir','reverse');
        xlabel('Frequency (ppm)','FontSize',10);
        ylabel('Amplitude(a.u.)','FontSize',10);
        ...
        box off;
        subplot(2,2,3);  
        plot(out_ms.ppm,real(out_ms.specs(:,:,2)));xlim([1 5]);
        set(gca,'FontSize',8);
        set(gca,'XDir','reverse');
        xlabel('Frequency (ppm)','FontSize',10);
        ylabel('Amplitude(a.u.)','FontSize',10);
        ...
        h=figure('visible','off');
        plot(1:out_ms.sz(out_ms.dims.averages),metric{1}(:,1),'.r','MarkerSize',16)
        hold on
        plot(1:out_ms.sz(out_ms.dims.averages),metric{1}(:,2),'.b','MarkerSize',16)
        removedAvgs = find(~ismember(1:out_ms.sz(out_ms.dims.averages),trackRemovedAvg));
...
if driftCorr=='n' || driftCorr=='N'
    ...
    if water
        outw_aa=op_alignAverages(outw_ms,0.2,'n');
    end
    sat='n';
    out_rm2=out_rm;
