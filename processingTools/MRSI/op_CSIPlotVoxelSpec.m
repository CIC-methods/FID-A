% op_CSIPlotVoxelSpec.m
% Brenden Kadota, SunnyBrook Hosptial 2021.
%
% USAGE:
% [out]=op_CSIPlotVoxelSpec(in, x, y, ppmmin, ppmmax, xlab, ylab, title);
% 
% DESCRIPTION:
% Plots the spec of one voxel in the CSI FID-A structure. Additional
% arguments augments the output plot. Uses the op_plotspec function.
% 
% INPUTS:
% in   = FID-A CSI structure
% x    = x index
% y    = y index
% ppmmin = min ppm value to plot
% ppmmax = max ppm value to plot
% xlab = x label for the plot
% ylab = y label fro the plot
% title = title
%
% OUTPUTS:
% fig   = figure handle
