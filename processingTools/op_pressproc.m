...
% 
% DESCRIPTION:
% Processing script for Siemens PRESS MRS data.  Includes combination of reciever channels,
...
function [out,out_w,out_noproc,out_w_noproc]=op_pressproc(filestring,aaDomain,tmaxin,iterin);
...
wrt=input('write? ','s');
if wrt=='y' || wrt=='Y'
    RF=op_writelcm(out,[filestring '/' filestring '_DriftCorr'],8.5);
    RF=op_writelcm(out_noproc,[filestring '/' filestring '_noDriftCorr'],8.5);
    if exist(filename2)
        RF=op_writelcm(out_w,[filestring '_w/' filestring '_w_DriftCorr'],8.5);
        RF=op_writelcm(out_w_noproc,[filestring '_w/' filestring '_w_noDriftCorr'],8.5);
    end
end
