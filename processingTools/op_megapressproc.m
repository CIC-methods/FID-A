...
% 
% DESCRIPTION:
% Processing script for Siemens MEGA-PRESS MRS data.  Includes combination of reciever channels,
% removal of bad averages, freqeuncy drift correction, manual alignment 
% of edit-on and edit-off spectra, and leftshifting.
% 
% INPUTS:
...
function [out1_diff,out1_sum,outw,out1,out1_cc,out1_rm,out1_aa]=op_megapressproc(filestring,avgAlignDomain,alignSS);
...
raw1=op_loadspec_twix([filestring '/' filename1]);
if water
    raww=op_loadspec_twix(filenamew);
end
...
if writ=='y' || writ=='Y'
    RF=op_writejmrui(out1_diff,[filestring '/MPdiff_full.txt']);
    RF=op_writejmrui(out1_sum,[filestring '/MPsum_full.txt']);
    if water
        RF=op_writejmrui(outw,[filestring '/MPw.txt']);
    end
end
