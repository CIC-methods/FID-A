...
%
%USAGE:
%[out,out_w,out_noproc,out_w_noproc]=op_specialproc(filestring,aaDomain,tmaxin,iterin);
%
%DESCRIPTION:
%Processing script for Siemens SPECIAL MRS data.  Includes combination of reciever channels,
...
function [out,out_w,out_noproc,out_w_noproc]=op_specialproc(filestring,aaDomain,tmaxin,iterin);
...
%normalize out number of averages:
...
wrt=input('write? ','s');
if wrt=='y' || wrt=='Y'
    RF=op_writelcm(out,[filestring '/' filestring '_lcm'],8.5);
    RF=op_writelcm(out_noproc,[filestring '/' filestring '_lcm_unprocessed'],8.5);
    if exist(filename2)
        RF=op_writelcm(out_w,[filestring '_w/' filestring '_w_lcm'],8.5);
        RF=op_writelcm(out_w_noproc,[filestring '_w/' filestring '_w_unprocessed'],8.5);
    end
end
